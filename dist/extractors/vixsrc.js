...
class VixSrc extends models_1.VideoExtractor {
    constructor(proxyConfig, adapter) {
        super(proxyConfig, adapter);
        this.serverName = 'VixSrc';
        this.sources = [];
        this.extract = async (videoUrl) => {
            ...
            const apiResp = await axios_1.default.get(`${BASE}${apiPath}`, {
                timeout: 10000,
                headers: {
            ...
            const { data: embedHtml } = await axios_1.default.get(embedUrl, {
                timeout: 10000,
                responseType: 'text',
            ...
            if (thumbnails) {
                source.thumbnails = thumbnails;
            }
            return source;
        };
    }
    toApiPath(videoUrl) {
        if (!videoUrl.hostname.endsWith('vixsrc.to'))
            return null;
    ...
    parseSubtitles(html) {
        ...
        if (!subSection)
            return out;
        try {
            const parsed = JSON.parse(subSection[1]);
